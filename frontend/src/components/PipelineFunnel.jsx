...
import { Search, Binary, ShieldCheck, Sparkles, Send, BarChart3, ArrowRight } from 'lucide-react';
...
export const PipelineFunnel = ({ stats = {} }) => {
  ...
  const stages = [
    {
      id: 1,
      label: '1. Discovery',
      count: `${pipeline.discovery ?? stats.total_leads ?? 0} found`,
      icon: Search,
    ...
    },
    {
      id: 2,
      label: '2. Extraction',
      count: `${stats.websites_processed ?? pipeline.discovery ?? stats.total_leads ?? 0} extracted`,
      icon: Binary,
    ...
    {
      id: 6,
      label: '6. Tracking & Reports',
      count: `${stats.campaigns_count ?? 1} reports`,
      icon: BarChart3,
  ...
  return (
    <div className="bg-[#0F172A] border border-[rgba(148,163,184,0.12)] rounded-xl p-5 mb-6 shadow-sm overflow-x-auto">
      <div className="flex items-center justify-between min-w-[880px] gap-2">
          ...
          return (
            <React.Fragment key={stage.id}>
              <motion.div 
                ...
                className="flex items-center gap-2.5 bg-[#080D1D] px-3 py-2.5 rounded-lg border border-[rgba(148,163,184,0.12)] flex-1"
              >
